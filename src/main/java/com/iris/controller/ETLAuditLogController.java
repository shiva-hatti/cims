...
public class ETLAuditLogController {
	...
	public ServiceResponse getAuditLogEntrybyFilingNumber(@RequestHeader(name = "AppId") String appId, @PathVariable(name = "uploadId") Long uploadId) {
		try {
			...
			if (CollectionUtils.isEmpty(etlAuditLogList)) {
				return new ServiceResponse.ServiceResponseBuilder().setStatus(false).setStatusCode(ErrorConstants.DEFAULT_ERROR.getConstantVal()).setStatusMessage("No entry in Audit Log table for given upload Id").build();
			}
			etlAuditLogList = prepareEtlAuditLogList(etlAuditLogList);
			return new ServiceResponseBuilder().setStatus(true).setStatusCode("200").setStatusMessage("Requet processed successfully").setResponse(etlAuditLogList).build();
		} catch (Exception e) {
	...
	@ResponseBody
	public ServiceResponse updateAuditLogEntry(@RequestHeader(name = "AppId") String appId, @PathVariable(name = "auditLogId") Long auditLogId, @PathVariable(name = "status") Long status) {
		try {
			...
			if (CollectionUtils.isEmpty(etlAuditLogList)) {
				return new ServiceResponse.ServiceResponseBuilder().setStatusCode(ErrorConstants.DEFAULT_ERROR.getConstantVal()).setStatus(false).setStatusMessage("No entry in Audit Log table for given upload Id").build();
			}
			etlAuditLogList = prepareEtlAuditLogList(etlAuditLogList);
			return new ServiceResponseBuilder().setStatus(true).setStatusCode("200").setStatusMessage("Record updated successfully.").setResponse(etlAuditLogList).build();
		} catch (Exception e) {
			LOGGER.error(ErrorConstants.ERROR_MSG_SERVICE.getConstantVal(), e);
			return new ServiceResponseBuilder().setStatus(false).setStatusCode(ErrorConstants.DEFAULT_ERROR.getConstantVal()).build();
		}
