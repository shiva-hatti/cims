...
public class CurrencyMasterController {
	...
	@GetMapping(value = "/isCheckCurrencyCodeListExit/{param1}/{param2}")
	public ServiceResponse isCheckCurrencyCodeListExit(@PathVariable("param1") String param1, @PathVariable("param2") String param2) {
		ServiceResponse serviceResponse = null;
		Logger.info("fetch isCheckCurrencyCodeListExit data controller started");
		List<String> shortNameList = new ArrayList<>();
		shortNameList.add(param1);
		shortNameList.add(param2);
		List<Currency> currencyList = currencyRepo.findCurrencyListByShortName(shortNameList);
		if (UtilMaster.isEmpty(currencyList)) {
			Logger.error("Data not found base upon currency " + param1 + "," + param2);
			return new ServiceResponseBuilder().setStatus(false).setStatusCode(ErrorCode.E1688.toString()).setStatusMessage(ObjectCache.getErrorCodeKey(ErrorCode.E1688.toString())).build();
		} else {

			List<String> currencyShortNameList = new ArrayList<>();
			for (Currency currency : currencyList) {
				currencyShortNameList.add(currency.getCurrencyShortName());
			}
			if (currencyShortNameList.containsAll(shortNameList)) {
				return new ServiceResponseBuilder().setStatus(true).setStatusCode(ErrorCode.E1688.toString()).setStatusMessage(ObjectCache.getErrorCodeKey(ErrorCode.E1688.toString())).build();
			} else {
				return new ServiceResponseBuilder().setStatus(false).setStatusCode(ErrorCode.E1688.toString()).setStatusMessage(ObjectCache.getErrorCodeKey(ErrorCode.E1688.toString())).build();
			}
		}
