...
public class SdmxReturnPreviewPublishController {
	...
	@Value("${scheduler.code.returnPreviewPublish}")
				...
				if (!CollectionUtils.isEmpty(sdmxReturnPreviewBeanList)) {
					...
					for (SdmxReturnPreviewBean sdmxReturnPreviewBean : sdmxReturnPreviewBeanList) {
						...
						try {
							...
							// Fetch property from DB table
							boolean submitDataToFusion = false;
							String submitDataToFusionString = fusionApiService.getFusionPropertyValue(FusionPropertiesConstant.SUBMIT_DATA_TO_FUSION.getConstant());
							if (!StringUtils.isBlank(submitDataToFusionString) && Boolean.TRUE.toString().equalsIgnoreCase(submitDataToFusionString)) {
								submitDataToFusion = Boolean.TRUE;
							}
							LOGGER.debug(GeneralConstants.JOB_PROCESSING_ID.getConstantVal() + " " + jobProcessingId + ", Submit data to fusion = " + submitDataToFusion);
							...
							if (processStatus || !submitDataToFusion) {
								...
								sdmxReturnPreviewBean.setEbrVersion(null);
								sdmxReturnPreviewBean.setValidFromDate(null);
								// Histry table status
							...
							// Formula JSON local
							if (sdmxReturnPreviewBean.getIsPublished()) {
								...
								sdmxReturnPreviewBean.setFormulaJson(gson.toJson(returnFormulaBeans));

								// Formula JSON Outer Use
								SdmxReturnFormulaMappingEntity sdmxReturnFormulaMappingEntity = new SdmxReturnFormulaMappingEntity();
								sdmxReturnFormulaMappingEntity.setFormulaJson(gson.toJson(returnFormulaBeans));
								...
								sdmxReturnFormulaMappingEntity.setReturnCode(sdmxReturnPreviewBean.getReturnCode());
