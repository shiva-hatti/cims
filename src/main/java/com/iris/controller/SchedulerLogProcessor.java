...
public class SchedulerLogProcessor {
	...
	public List<SchedulerInfo> getSchedulerLogList(int count) {
		...
		sb.append("	select  SCHEDULER_ID,SCHEDULER_NAME,RECORDS_TO_BE_PROCESSED,IS_ACTIVE,SCHEDULER_CODE,MAX_PARALLEL_INSTANCE," + "ID,SCHEDULER_ID_FK,SCHEDULER_STARTED_TIME,SCHEDULER_STOPPED_TIME,TAKEN_RECORDS_COUNT,SUCCESSFULLY_PROCESSED_COUNT,FAILED_RECORD_COUNT,JOB_PROCESS_ID," + "IP_ADDRESS,IS_RUNNING,rn from TBL_SCHEDULER a LEFT JOIN( ").append(" select * from  ( select T.SCHEDULER_ID_FK,T.ID,T.SCHEDULER_STARTED_TIME,T.SCHEDULER_STOPPED_TIME,T.TAKEN_RECORDS_COUNT,T.SUCCESSFULLY_PROCESSED_COUNT,T.FAILED_RECORD_COUNT,T.JOB_PROCESS_ID,T.IP_ADDRESS,T.IS_RUNNING, ").append(" row_number() over(partition by T.SCHEDULER_ID_FK order by T.ID desc) as rn from TBL_SCHEDULER_LOG as T ").append("  ) as T where T.rn <= :count ) b  on a.SCHEDULER_ID=b.SCHEDULER_ID_FK ");
		...
		try {
			...
			for (Tuple tuple : result) {
				...
				if (schedulerLogMap.containsKey(tuple.get("SCHEDULER_ID"))) {
					if (schedulerLogMap.get(tuple.get("SCHEDULER_ID")) != null) {
						...
						if (schedulerInfo.getSchedulerLoglist() != null) {
							...
							if (tuple.get("FAILED_RECORD_COUNT") != null) {
								schedulerLogInfo.setFailedProcessCount(Integer.parseInt(tuple.get("FAILED_RECORD_COUNT").toString()));
							}
							...
							if (tuple.get("IS_RUNNING") != null && tuple.get("IS_RUNNING").toString().equalsIgnoreCase("1")) {
								schedulerLogInfo.setIsRunning(Boolean.TRUE);
								isRunningMain = true;
							} else {
								schedulerLogInfo.setIsRunning(Boolean.FALSE);
							}
					...
					if (tuple.get("MAX_PARALLEL_INSTANCE") != null) {
						schedulerInfo.setMaxParallelInstance(Integer.parseInt(tuple.get("MAX_PARALLEL_INSTANCE").toString()));
					}
				...
				schedulerInfo = schedulerLogMap.get(tuple.get("SCHEDULER_ID"));
				if (schedulerInfo != null && schedulerInfo.getIsRunning() != null && !schedulerInfo.getIsRunning()) {
					schedulerInfo.setIsRunning(isRunningMain);
				}
			}
