...
public interface UserRoleReturnMappingRepo extends JpaRepository<UserRoleReturnMapping, Long> {
	...
	@Query(value = "SELECT DISTINCT new com.iris.dto.OwnerReturn(retMap.returnIdFk.returnId) from UserRoleReturnMapping retMap, Return ret where retMap.roleIdFk.userRoleId IN(:roleIdSet) and retMap.isActive =:isActive and retMap.returnIdFk.returnId = ret.returnId and ret.isActive = :isActive and ret.frequency.isActive = :isActive and ret.returnGroupMapIdFk.isActive =:isActive")
	Set<OwnerReturn> getUserRoleReturnDataByRoleIdAndActiveFlag(@QueryParam("roleIdSet") Set<Long> roleIdSet, @QueryParam("isActive") boolean isActive);
