...
public interface ReturnRegulatorMappingRepo extends JpaRepository<ReturnRegulatorMapping, Long> {
	...
	@Query(value = "SELECT DISTINCT new com.iris.dto.OwnerReturn(reg.returnIdFk.returnId) from ReturnRegulatorMapping reg, UserRoleReturnMapping retMap, Return ret where retMap.roleIdFk.userRoleId IN(:roleIdSet) and retMap.isActive = :isActive and retMap.returnIdFk.returnId = ret.returnId and reg.regulatorIdFk.regulatorId =:reguLatorId and reg.returnIdFk.returnId = retMap.returnIdFk.returnId and reg.isActive = :isActive and ret.isActive = :isActive and ret.frequency.isActive = :isActive and ret.returnGroupMapIdFk.isActive =:isActive")
	Set<OwnerReturn> getMappedReturnListByRegulatorIdAndIsActiveStatus(@Param("roleIdSet") Set<Long> roleIdSet, @Param("reguLatorId") Long reguLatorId, @Param("isActive") boolean isActive);
