...
public interface ReturnTemplateRepository extends JpaRepository<ReturnTemplate, Long> {
	...
	@Query(value = "select RETURN_TEMPLATE_ID FROM TBL_RETURN_TEMPLATE WHERE RETURN_ID_FK = :returnId AND SAVE_FORMULA_AS_DRAFT =:saveAsDraft AND IS_ACTIVE =:isActive " + " ORDER BY RETURN_TEMPLATE_ID DESC LIMIT 1", nativeQuery = true)
	Long findReturnTemplateIdByReturnIdAndSaveAsDraftIsActiveFalse(@Param("returnId") Long returnId, @Param("saveAsDraft") Boolean saveAsDraft, @Param("isActive") Boolean isActive);
}