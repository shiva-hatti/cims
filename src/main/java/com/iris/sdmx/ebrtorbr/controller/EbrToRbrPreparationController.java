...
public class EbrToRbrPreparationController {
	...
	public void watchForAnySDMXFilingReceived() {
		jobProcessingId = UUID.randomUUID().toString();
		LOGGER.info("Scheduler " + schedulerCode + " started with Job processing ID {}", jobProcessingId);
		Long schedulerLogId = null;
		Scheduler scheduler = getSchedulerStatus(schedulerCode);

		if (scheduler != null) {
			if (scheduler.getIsRunning().equals(Boolean.TRUE)) {
				LOGGER.error("Error while starting scheduler " + schedulerCode + " -> Reason : Schduler is alrady running ");
				return;
			}
			try {
				schedulerLogId = makeSchedulerStartEntry(0L, scheduler.getSchedulerId());
				if (StringUtils.isEmpty(schedulerLogId)) {
					LOGGER.error("Error while starting scheduler  " + schedulerCode + " -> Reason : Schduler Log ID not received ");
					return;
				}
			} catch (Exception e) {
				LOGGER.error(ErrorConstants.DEFAULT_ERROR.getConstantVal(), e);
			...
			makeSchedulerStopEntry(totalRecordCount, totalProcessedRecord, schedulerLogId, scheduler.getSchedulerId(), schedulerCode);
		} else {
			LOGGER.error(ErrorConstants.SCHEDULER_INFO_NOT_PRESENT.getConstantVal());
		}
	}
