...
public class SdmxRetrunPreviewValidator {
	 ...
	 * @throws ApplicationException
	 */
	public void validateFetchHtmlById(Long userId, String jobProcessId, Long returnTemplateId) throws ApplicationException {
	...
	public void validatePublishReturnTemplate(Long userId, String jobProcessId, SdmxReturnPreviewBean sdmxReturnPreviewBean) throws ApplicationException {
		...
		//validateElements(sdmxReturnPreviewBean.getReturnPreviewTypeId());
	...
	public void validateActivateDeactivateTemplate(Long userId, String jobProcessId, SdmxReturnPreviewBean sdmxReturnPreviewBean) throws ApplicationException {
		LOGGER.info("START - Validation request to publish return template with Job Processing ID : " + jobProcessId);
		...
		validateModuleActiveStatus(sdmxReturnPreviewBean.getModuleActiveStatus());

		if (SDMXConstants.ModuleActiveStatus.ACTIVE.getModuleStatusCode().equalsIgnoreCase(sdmxReturnPreviewBean.getModuleActiveStatus()) && sdmxReturnPreviewEntity.getIsPublished().equals(Boolean.TRUE)) {
			validateFromDate(sdmxReturnPreviewBean.getValidFromDate());
			validateDesc(sdmxReturnPreviewBean.getDescription());
		}
	...
	public void validateCellCalculation(Long userId, String jobProcessId, PreviewPublishBean previewPublishBean) throws ApplicationException {
	...
	public void validateGetTemplateFileBasePath(Long userId, String jobProcessId) throws ApplicationException {
	...
	public void validateModuleActiveStatus(String moduleActiveStatus) throws ApplicationException {
		if (StringUtils.isBlank(moduleActiveStatus) || !(SDMXConstants.ModuleActiveStatus.ACTIVE.getModuleStatusCode().equalsIgnoreCase(moduleActiveStatus) || SDMXConstants.ModuleActiveStatus.DEACTIVE.getModuleStatusCode().equalsIgnoreCase(moduleActiveStatus))) {
	...
	public void validateElements(Long returnPreviewTypeId) throws ApplicationException {
		List<SdmxModelCodesEntity> sdmxModelCodesEntityList = new ArrayList<>();
		Map<String, String> dsdCodeVersionMap = new HashMap<>();

		sdmxModelCodesEntityList = sdmxModelCodesRepo.fetchDistinctElementOfDataTemplate(returnPreviewTypeId);
		if (!sdmxModelCodesEntityList.isEmpty()) {
			String dsdCode;
			String version;
			for (SdmxModelCodesEntity sdmxModelCodesEntity : sdmxModelCodesEntityList) {
				dsdCode = sdmxModelCodesEntity.getElementIdFk().getDsdCode();
				version = sdmxModelCodesEntity.getElementIdFk().getElementVer();
				if (dsdCodeVersionMap.containsKey(dsdCode)) {
					if (!dsdCodeVersionMap.get(dsdCode).equals(version)) {
						throw new ApplicationException(ErrorCode.E1245.toString(), ObjectCache.getErrorCodeKey(ErrorCode.E1245.toString()));
					}
				} else {
					dsdCodeVersionMap.put(dsdCode, version);
				}
