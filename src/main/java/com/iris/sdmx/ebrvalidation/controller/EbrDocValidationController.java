...
public class EbrDocValidationController {
	...
	public ServiceResponse fetchEbrRecordListForValidation(@RequestHeader("JobProcessingId") String jobProcessId, @RequestBody EbrFileDetailsInputRequest ebrFileDetailsInputRequest) {
		try {
	...
	@PostMapping(value = "/fetchElementDimensionMapping")
	public ServiceResponse fetchElementDimensionMapping(@RequestHeader("JobProcessingId") String jobProcessId, @RequestBody List<SdmxModelCodeLiteBean> sdmModelCodeLiteBeans) {
		try {
			validateRequestForFetchElementDimensionMapping(sdmModelCodeLiteBeans, jobProcessId);

			List<ElementDimensionStoredJson> elemetDimensionStoredJsons = ebrDocValidationService.fetchElementDimenionMappingData(sdmModelCodeLiteBeans, jobProcessId);

			return new ServiceResponseBuilder().setStatus(true).setStatusCode(com.iris.util.constant.GeneralConstants.SUCCESS.getConstantVal()).setResponse(elemetDimensionStoredJsons).build();
		} catch (Exception e) {
			LOGGER.error(ErrorConstants.ERROR_MSG_CONTROLLER.getConstantVal() + "Transaction ID : " + jobProcessId, e);
			return new ServiceResponseBuilder().setStatus(false).setStatusCode(ErrorCode.EC0013.toString()).setStatusMessage(ObjectCache.getErrorCodeKey(ErrorCode.EC0013.toString())).build();
		}
	...
	private void validateRequestForFetchElementDimensionMapping(List<SdmxModelCodeLiteBean> sdmModelCodeLiteBeans, String jobProcessId) throws ApplicationException {
