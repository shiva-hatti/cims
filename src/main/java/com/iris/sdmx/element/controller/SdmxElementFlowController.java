...
public class SdmxElementFlowController {
	...
	public ServiceResponse fetchElementFlow(@RequestHeader("JobProcessingId") String jobProcessId, @RequestHeader("IsActive") Boolean isActive, @PathVariable("userId") Long userId, @RequestBody SdmxElementFlowTypeBean sdmxElementFlowTypeBeans) {
		LOGGER.info("START - Fetch Element Flow by Active status request received with Job Processing ID : " + jobProcessId);
		...
		LOGGER.info("End - Fetch Element Flow by Active status request completed with Job Processing ID : " + jobProcessId);
