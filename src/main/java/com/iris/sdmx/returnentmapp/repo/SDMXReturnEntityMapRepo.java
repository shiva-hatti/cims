...
public interface SDMXReturnEntityMapRepo extends JpaRepository<SDMXReturnEntityMapp, Long> {
	...
	@Query(value = " select new com.iris.sdmx.returnentmapp.bean.SDMXReturnEntityMapp(ret.returnId, ret.returnCode, ent.entityId, ent.entityCode, ret.returnPropertyIdFk.returnProprtyId) " + " from Return ret, SDMXReturnEntityMapp map, EntityBean ent where map.isActive = 1 " + " and ent.isActive = 1 and ret.isActive = 1 " + " and ret.returnId =  map.returnObj.returnId and map.entity.entityId = ent.entityId and ret.returnId in (:returnIds)")
