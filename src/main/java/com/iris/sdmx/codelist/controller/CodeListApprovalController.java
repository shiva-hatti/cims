...
public class CodeListApprovalController {
	...
	public ServiceResponse approveRejectCodeList(@RequestHeader("JobProcessingId") String jobProcessId, @RequestBody ApprovalInputBean approvalInputBean) {
	...
	public ServiceResponse isCodeListArrayPending(@RequestHeader(name = "JobProcessingId") String jobProcessId, @RequestBody List<CodelListMasterRequestBean> codeListMasterRequestBean, @PathVariable("userId") Long userId) {
		LOGGER.info("Request received to check code list master pending or not for job processing Id : " + jobProcessId + " With Input request : " + JsonUtility.getGsonObject().toJson(codeListMasterRequestBean));
