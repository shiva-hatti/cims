...
public class SdmxFileActivityLogHelper {
	...
	public void convertRequestToEntity(SdmxActivityDetailLogRequest sdmxActivityDetailLogRequest, SdmxFileActivityLog sdmxActivityDetailLog, Long userId, String jobProcessingId) {
		LOGGER.debug("START - Convert request to entity for add Sdmx Activity Detail Log record request received with Job Processing ID : " + jobProcessingId);
		...
		if (!StringUtils.isBlank(sdmxActivityDetailLogRequest.getProcessCode())) {
			SdmxProcessDetailEntity sdmxProcessDetailEntity = sdmxProcessDetailRepo.findProcessByProcessCode(sdmxActivityDetailLogRequest.getProcessCode());
		...
		LOGGER.debug("END - Convert request to entity for add Sdmx Activity Detail Log record request received with Job Processing ID : " + jobProcessingId);
	}

	public List<SdmxFileActivityLog> convertRequestToList(List<SdmxActivityDetailLogRequest> sdmxActivityDetailLogRequestList, String jobProcessingId) {
		LOGGER.debug("START - Convert request to entity for add Sdmx Activity Detail Log record request received with Job Processing ID : " + jobProcessingId);

		List<SdmxFileActivityLog> sdmxFileActivityLogList = new ArrayList<>();
		for (SdmxActivityDetailLogRequest sdmxActivityDetailLogRequest : sdmxActivityDetailLogRequestList) {
			SdmxFileActivityLog sdmxActivityDetailLog = new SdmxFileActivityLog();
			BeanUtils.copyProperties(sdmxActivityDetailLogRequest, sdmxActivityDetailLog);
			...
			// User
			if (sdmxActivityDetailLogRequest.getUserId() != null) {
				UserMaster userMaster = new UserMaster();
				userMaster.setUserId(sdmxActivityDetailLogRequest.getUserId());
				sdmxActivityDetailLog.setCreatedBy(userMaster);
				// Created On
				sdmxActivityDetailLog.setCreatedOn(new Date());
			}
			...
			sdmxActivityDetailLog.setJobProcessingId(jobProcessingId);
			sdmxFileActivityLogList.add(sdmxActivityDetailLog);
		}
		LOGGER.debug("END - Convert request to entity for add Sdmx Activity Detail Log record request received with Job Processing ID : " + jobProcessingId);
		return sdmxFileActivityLogList;
	}
	 ...
	 */
	public void convetEntityToBean(SdmxFileActivityLog sdmxActivityDetailLog, Long userId, String jobProcessingId, SdmxFileActivityLogBean sdmxActivityDetailLogBean) {
		LOGGER.debug("START - Convert entity to bean for add Sdmx Activity Detail Log record request received with Job Processing ID : " + jobProcessingId);
		...
		LOGGER.debug("END - Convert entity to bean for add Sdmx Activity Detail Log record request received with Job Processing ID : " + jobProcessingId);
	}

	public List<SdmxFileActivityLogBean> convetActivityLogListToBeanList(List<SdmxFileActivityLog> sdmxActivityDetailLogList, String jobProcessingId) {
		LOGGER.debug("START - Convert entity to bean for add Sdmx Activity Detail Log record request received with Job Processing ID : " + jobProcessingId);
		List<SdmxFileActivityLogBean> sdmxFileActivityLogBeanList = new ArrayList<>();
		for (SdmxFileActivityLog sdmxActivityDetailLog : sdmxActivityDetailLogList) {
			...
			sdmxFileActivityLogBeanList.add(sdmxActivityDetailLogBean);
		}

		LOGGER.debug("END - Convert entity to bean for add Sdmx Activity Detail Log record request received with Job Processing ID : " + jobProcessingId);
		return sdmxFileActivityLogBeanList;
	}
