...
public interface DynamicDropDownRepo extends JpaRepository<DropDownValues, Integer> {
	...
	@Query("FROM DropDownValues where dropDownTypeIdFk.dropdownTypeCode = :dropDownType order by dropDownValueId")
	List<DropDownValues> getReportingMonth(@Param("dropDownType") String dropDownType);

	@Query("FROM DropDownValues where dropDownTypeValueIdFk.dropDownValueId IN (select dropDownValueId from DropDownValues where dropDownTypeIdFk.dropdownTypeId = :dropDownValueIdFk) order by dropDownValueId")
	List<DropDownValues> findSubDropDownValue(@Param("dropDownValueIdFk") Long dropDownValueIdFk);
