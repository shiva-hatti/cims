...
public class ElementDimensionMappController {
	...
	public ServiceResponse getElementdimensionMappingData(@RequestHeader(name = "JobProcessingId") String jobProcessId, @PathVariable("userId") Long userId, @PathVariable("roleId") Long roleId, @PathVariable("langCode") String langCode) {
	...
	public ServiceResponse getElementdimensionMappingWithCodeList(@RequestHeader(name = "JobProcessingId") String jobProcessId, @PathVariable("userId") Long userId, @PathVariable("elementId") Long elementId) {
		try {
			List<ElementDimensionCodeListBean> elementDimensionCodeListBeanList = elementDimensionService.findByElement(elementId);

			return new ServiceResponseBuilder().setStatus(true).setStatusMessage(ErrorConstants.RECORD_SAVED_SUCCESSFULLY.getErrorMessage()).setResponse(elementDimensionCodeListBeanList).build();
		} /*
			 * catch (ApplicationException e) {
	...
	public ServiceResponse getElementdimensionMappingWithCodeListValue(@RequestHeader(name = "JobProcessingId") String jobProcessId, @PathVariable("userId") Long userId, @PathVariable("elementId") Long elementId) {
		try {
			List<ElementDimensionCodeListValueBean> elementDimensionCodeListBeanList = elementDimensionService.findByElementCodeListValue(elementId);
	...
	public ServiceResponse addElementDimensionMappingData(@RequestHeader(name = "JobProcessingId") String jobProcessId, @RequestBody ElementDimensionBean elementDimensionBean) {
		...
		try {
			...
			int isDeptAdmin = Integer.parseInt(userRoleRepo.findByUserRoleIdAndIsActiveTrue(elementDimensionBean.getRoleId()).getDeptAdmin());

			if (isDeptAdmin != 1) {
				return new ServiceResponseBuilder().setStatus(false).setStatusCode("ERROR").setStatusMessage("User is not authorized to perform this action").build();
			}

			if (elementDimensionService.addElementDimensionMapData(elementDimensionBean, elementDimensionBean.getAddApproval())) {
	...
	public ServiceResponse editElementDimensionMappingData(@RequestHeader(name = "JobProcessingId") String jobProcessId, @RequestBody ElementDimensionBean elementDimensionBean) {
		...
		try {
			...
			if (elementDimensionService.editElementDimensionData(elementDimensionBean, elementDimensionBean.getEditApproval())) {
				return new ServiceResponseBuilder().setStatus(true).setStatusMessage(ErrorConstants.RECORD_SAVED_SUCCESSFULLY.getErrorMessage()).build();
			}
		} catch (ApplicationException e) {
			LOGGER.error("Exception occured for job processing Id : " + jobProcessId + "", e);
	...
	@DeleteMapping(value = "/deleteElementDimensionMappingData/{userId}/{roleId}/{langCode}/{elementCode}/{elementVersion}/{agencyCode}")
	public ServiceResponse deleteElementDimensionMappingData(@RequestHeader(name = "JobProcessingId") String jobProcessId, @RequestHeader(name = "deleteApproval") String deleteApproval, @PathVariable(name = "elementCode") String elementCode, @PathVariable(name = "agencyCode") String agencyCode, @PathVariable(name = "elementVersion") String elementVersion, @PathVariable(name = "userId") Long userId, @PathVariable(name = "roleId") Long roleId, @PathVariable("langCode") String langCode) {
		LOGGER.info("Request received to get element dimensionapping for jpb processing id :" + jobProcessId);
		...
		try {
			...
			isApprovalRequired = Boolean.parseBoolean(deleteApproval);
			if (elementDimensionService.deleteElementDimensionMappingData(elementCode, elementVersion, agencyCode, isApprovalRequired, userId)) {
				return new ServiceResponseBuilder().setStatus(true).setStatusMessage(ErrorConstants.RECORD_SAVED_SUCCESSFULLY.getErrorMessage()).build();
			}
		} catch (ApplicationException e) {
			LOGGER.error("Exception occured for job processing Id : " + jobProcessId + "", e);
	...
	private void validateInputRequestToDeleteElementDimensionData(String jobProcessingId, String elementCode, String elementVersion, Long userId, Long roleId) throws ApplicationException {
		if (UtilMaster.isEmpty(jobProcessingId) || UtilMaster.isEmpty(userId) || UtilMaster.isEmpty(roleId) || UtilMaster.isEmpty(elementCode) || UtilMaster.isEmpty(elementVersion)) {
			throw new ApplicationException(ErrorCode.E0889.toString(), ObjectCache.getErrorCodeKey(ErrorCode.E0889.toString()));
		}
	}

	private void validateInputRequestToAddElementDimensionData(String jobProcessingId, ElementDimensionBean elementDimensionBean) throws ApplicationException {
		if (UtilMaster.isEmpty(jobProcessingId) || UtilMaster.isEmpty(elementDimensionBean.getUserId()) || UtilMaster.isEmpty(elementDimensionBean.getRoleId()) || UtilMaster.isEmpty(elementDimensionBean.getDsdCode()) || UtilMaster.isEmpty(elementDimensionBean.getElementVersion())
	...
	private void validateInputRequestToEditElementDimensionData(String jobProcessingId, ElementDimensionBean elementDimensionBean) throws ApplicationException {
		if (UtilMaster.isEmpty(jobProcessingId) || UtilMaster.isEmpty(elementDimensionBean.getUserId()) || UtilMaster.isEmpty(elementDimensionBean.getRoleId()) || UtilMaster.isEmpty(elementDimensionBean.getDsdCode()) || UtilMaster.isEmpty(elementDimensionBean.getElementVersion())
		/* || CollectionUtils.isEmpty(elementDimensionBean.getDimensionmasterBeans()) */) {
			throw new ApplicationException(ErrorCode.E0889.toString(), ObjectCache.getErrorCodeKey(ErrorCode.E0889.toString()));
		}
	...
	public ServiceResponse getAttachementType(@PathVariable(name = "userId") Long userId, @PathVariable(name = "roleId") Long roleId, @PathVariable("langCode") String langCode) {
		try {
	...
	public ServiceResponse isElementDimensionMapExist(@RequestHeader("JobProcessingId") String jobProcessId, @PathVariable("userId") Long userId, @PathVariable("elementId") Long elementId, @RequestBody SdmxElementExistBean sdmxElementExistBean) {
		LOGGER.info("START : Request received to check if Element-dimension map exist for jpb processing id :" + jobProcessId);
		...
		LOGGER.info("END : Request received to check if Element-dimension map exist for jpb processing id :" + jobProcessId);
