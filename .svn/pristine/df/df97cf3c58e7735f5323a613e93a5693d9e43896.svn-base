...
public class SdmxModelCodesHelper {
	 ...
	 */
	public static void convertEntityToBean(SdmxModelCodesEntity sdmxModelCodesEntity, SdmxModelCodesBean sdmxModelCodesBean) {
	 ...
	 * @param userId
	 */
	public static void convertBeanToEntity(SdmxModelCodesBean sdmxModelCodesBean, SdmxModelCodesEntity sdmxModelCodesEntity, Long userId) {
	 ...
	 * @param dimConceptId
	 */
	public void setCommonDimension(List<DimensionCodeListValueBean> commonDimension, String clValueCode, String dimConceptId) {
	 ...
	 */
	public void setOpenDimension(List<DimensionCodeListValueBean> openDimension, String clValueCode, String dimConceptId, List<DimensionCodeListValueBean> openDimensionForHash, List<String> openDimList) {
		clValueCode = Validations.trimInput(clValueCode);
		dimConceptId = Validations.trimInput(dimConceptId);
		DimensionCodeListValueBean dimensionCodeListValueBean = new DimensionCodeListValueBean();
		dimensionCodeListValueBean.setDimConceptId(dimConceptId);
		if (!StringUtils.isBlank(clValueCode)) {
			dimensionCodeListValueBean.setClValueCode(clValueCode);
			if (!(clValueCode.equalsIgnoreCase(SDMXConstants.ITEM_FREE_TEXT) || clValueCode.equalsIgnoreCase(SDMXConstants.FILER_TO_SELECT))) {
	...
	public void saveDMIDGrouping(SdmxEleDimTypeMapBean sdmxEleDimTypeMapBean, Gson gson) {
		...
		if (!isEleDimExist) {
			...
			Integer maxGroupNum = sdmxEleDimTypeMapService.fetchMaxGroupNumByEleNVersion(sdmxEleDimTypeMapBean.getDsdCode(), sdmxEleDimTypeMapBean.getElementVer());
			...
			if (!CollectionUtils.isEmpty(closeDimList)) {
				processDimGroupList(closeDimList, sdmxEleDimTypeMapBean, "CLOSED", eleDimHashString, maxGroupNum, sdmxEleDimTypeMapEntityList);
			}

			// Processing Open Dim
			List<String> openDimList = sdmxEleDimTypeMapBean.getOpenDim();
			if (!CollectionUtils.isEmpty(openDimList)) {
				processDimGroupList(openDimList, sdmxEleDimTypeMapBean, "OPEN", eleDimHashString, maxGroupNum, sdmxEleDimTypeMapEntityList);
			}
	 ...
	 */
	public void processDimGroupList(List<String> dimValueList, SdmxEleDimTypeMapBean sdmxEleDimTypeMapBean, String dimType, String eleDimHashString, Integer maxGroupNum, List<SdmxEleDimTypeMapEntity> sdmxEleDimTypeMapEntityList) {
	 ...
	 */
	public void setClosedDimension(List<DimensionCodeListValueBean> closedDim, String clValueCode, String dimConceptId, List<DimensionCodeListValueBean> closedDimForHash, List<String> closeDimList) {
		clValueCode = Validations.trimInput(clValueCode);
		dimConceptId = Validations.trimInput(dimConceptId);
		DimensionCodeListValueBean dimensionCodeListValueBean = new DimensionCodeListValueBean();
		dimensionCodeListValueBean.setDimConceptId(dimConceptId);
	...
	public Long fetchBeanFromRequest(ModelCodeInputBean modelCodeInputBean, Long userId, SdmxModelCodesBean sdmxModelCodesBean, SdmxEleDimTypeMapBean sdmxEleDimTypeMapBean) throws ApplicationException {
		Long modelCodeId = 0L;
		Gson gson = new Gson();
		...
		List<SdmxDimensionDetail> sdmxDimensionDetailList = (List<SdmxDimensionDetail>) new Gson().fromJson(new String(Base64.decodeBase64(modelCodeInputBean.getModelDim().getBytes())), type);
		...
		if (sdmxElementBean != null) {
			...
			for (SdmxDimensionDetail sdmxDimensionDetail : sdmxDimensionDetailList) {
				DimensionMaster dimensionMaster = dimensionRepo.getEntityUsingDimCode(sdmxDimensionDetail.getDimensionCode(), true, false, dimConceptIdNConceptVersion.get(sdmxDimensionDetail.getDimensionCode()), sdmxElementBean.getSdmxAgencyMasterBean().getAgencyMasterCode());
				if (dimensionMaster != null && (dimensionMaster.getCodeListMaster() == null || (!dimensionMaster.getCodeListMaster().getIsPending()))) {
					if (sdmxDimensionDetail.getIsCommon() != null && sdmxDimensionDetail.getIsCommon()) {
						// Set Common dimension
						setCommonDimension(commonDimension, sdmxDimensionDetail.getSelectedCodeListId(), sdmxDimensionDetail.getDimensionCode());
					} else if (sdmxDimensionDetail.getDimensionType().equalsIgnoreCase("OPEN")) {
						// Set Open Dimension
						setOpenDimension(openDimension, sdmxDimensionDetail.getSelectedCodeListId(), sdmxDimensionDetail.getDimensionCode(), openDimensionForHash, openDimList);

					} else {
						// Set Closed Dimension
						setClosedDimension(closedDim, sdmxDimensionDetail.getSelectedCodeListId(), sdmxDimensionDetail.getDimensionCode(), closedDimForHash, closeDimList);
					}
				} else {
					throw new ApplicationException(ErrorCode.E1619.name(), ObjectCache.getErrorCodeKey(ErrorCode.E1619.toString()), ObjectCache.getErrorCodeKey(ErrorCode.E1619.toString()));
				}
			...
			if (!CollectionUtils.isEmpty(closedDim)) {
				closedDim.sort((DimensionCodeListValueBean s1, DimensionCodeListValueBean s2) -> s1.getDimConceptId().compareTo(s2.getDimConceptId()));
			}
			if (!CollectionUtils.isEmpty(closedDimForHash)) {
				closedDimForHash.sort((DimensionCodeListValueBean s1, DimensionCodeListValueBean s2) -> s1.getDimConceptId().compareTo(s2.getDimConceptId()));
			}
			...
			if (!CollectionUtils.isEmpty(openDimension)) {
				openDimension.sort((DimensionCodeListValueBean s1, DimensionCodeListValueBean s2) -> s1.getDimConceptId().compareTo(s2.getDimConceptId()));
			}

			if (!CollectionUtils.isEmpty(openDimensionForHash)) {
				openDimensionForHash.sort((DimensionCodeListValueBean s1, DimensionCodeListValueBean s2) -> s1.getDimConceptId().compareTo(s2.getDimConceptId()));
				dimensionDetailCategories.setOpenDimension(openDimensionForHash);
			}

			ModelOtherDetails modelOtherDetails = new ModelOtherDetails();
			if (!StringUtils.isBlank(modelCodeInputBean.getIsDependentCell())) {
				modelOtherDetails.setDependencyType(modelCodeInputBean.getIsDependentCell());
			...
			SdmxModelCodesBean sdmxModelCodesBean2 = modelCodesService.findEntityByModelCodeHash(sdmxModelCodesBean.getModelDimHash());
			...
			throw new ApplicationException(ErrorCode.E1569.toString(), ObjectCache.getErrorCodeKey(ErrorCode.E1569.toString()));
		}
		return modelCodeId;
	}
