...
public class AuditFirmController {
	...
	public ServiceResponse addAuditFirm(@RequestHeader(name = "AppId") String jobProcessId, @RequestHeader(name = "UUID") String uuid, @RequestBody AuditFirm auditFirm) {
		LOGGER.info("Add/Edit Audit Firm: addAuditFirm");
		ServiceResponse serviceResponse = null;
		try {
			if (UtilMaster.isEmpty(auditFirm.getAuditFirmDefaultName())) {
				return new ServiceResponseBuilder().setStatus(false).setStatusCode(ErrorCode.E0182.toString()).setStatusMessage(ObjectCache.getErrorCodeKey(ErrorCode.E0182.toString())).build();
			}
			...
			if (UtilMaster.isEmpty(auditFirm.getBusinessAddress())) {
				return new ServiceResponseBuilder().setStatus(false).setStatusCode(ErrorCode.E0184.toString()).setStatusMessage(ObjectCache.getErrorCodeKey(ErrorCode.E0184.toString())).build();
			}
	...
	public ServiceResponse checkAuditFirmExists(@RequestHeader(name = "AppId") String jobProcessId, @RequestHeader(name = "UUID") String uuid, @PathVariable("firmRegNo") String firmRegNo) {
		...
		try {
			...
			if (auditFirmInfo != null) {
				return new ServiceResponseBuilder().setStatus(true).setStatusCode(ErrorCode.E0191.toString()).setStatusMessage(ObjectCache.getErrorCodeKey(ErrorCode.E0191.toString())).build();
			}
		} catch (Exception e) {
			LOGGER.error(ErrorCode.EC0033.toString());
			return new ServiceResponseBuilder().setStatus(false).setStatusCode(ErrorCode.EC0033.toString()).setStatusMessage(ObjectCache.getErrorCodeKey(ErrorCode.EC0033.toString())).build();
	...
	public ServiceResponse updateAuditFirmLabel(@RequestHeader(name = "AppId") String jobProcessId, @RequestHeader(name = "UUID") String uuid, @RequestBody AuditFirm auditFirm) {
