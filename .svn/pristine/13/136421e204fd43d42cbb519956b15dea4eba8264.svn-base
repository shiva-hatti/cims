...
public class BranchMasterDomesticController {
	 ...
	 * Added By psahoo
	 */
	@GetMapping("/getBranchDomesticDetails/{langCode}/{branchStatus}/{branchData}/{entityCode}")
	public ServiceResponse getBranchDomesticDetails(@PathVariable(name = "langCode") String langCode, @PathVariable(name = "branchStatus") String branchStatus, @PathVariable(name = "branchData") String branchData, @PathVariable(name = "entityCode") String entityCode) {
		...
		try {
			...
			if (Validations.isEmpty(langCode) || Validations.isEmpty(branchStatus) || Validations.isEmpty(branchData) || branchData.length() < 4 || Validations.isEmpty(entityCode)) {
			...
			if (branchStatus.equals(GeneralConstants.BRANCH_CODE.getConstantVal()) || branchStatus.equals(GeneralConstants.BRANCH_CODE_BRANCH_NAME.getConstantVal())) {
				listOfActiveBranchDomecticData = branchMasterDomesticRepo.getBranchMasterListByBranchCode(branchData.trim(),entityCode.trim());
				...
				listOfActiveBranchDomecticData = branchMasterDomesticRepo.getBranchMasterListByBranchName(branchData.trim(),entityCode.trim());
			}
