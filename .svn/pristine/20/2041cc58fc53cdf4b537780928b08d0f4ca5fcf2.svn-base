 ...
 */
@Component
public class SdmxElementAuditValidator implements Serializable {
	...
	private static final Logger LOGGER = LogManager.getLogger(SdmxElementAuditValidator.class);
	...
	public void validateFetchSdmxFileAuditRequest(String jobProcessId, Long userId, Integer statusId) throws ApplicationException {
		LOGGER.debug("Validation request start for fetch sdmx element audit request with job processing id - " + jobProcessId);
		// Job procession id
		...
		LOGGER.debug("Validation request end for fetch sdmx element audit request with job processing id - " + jobProcessId);
	}
