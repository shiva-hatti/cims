...
public class UserRoleService implements GenericService<UserRole, Long> {
	...
	public List<UserRole> getDataByIds(Long[] ids) throws ServiceException {
		try {
			return userRoleRepo.findByUserRoleIds(ids);
		} catch (Exception e) {
	...
	public List<Long> getActivityIdByRoleIds(List<Long> roleIds) {
		return userRoleRepo.getActivityIdByRoleIds(roleIds);
	}

	public List<UserDto> checkActivityMappedToLoggedInUser(Long userId, Long actityId) {
		return userRoleRepo.getRoleDetailsBeaseUponActivityAndUserId(userId, actityId);
	}
