...
public class WebformCustomDatesController {
	...
	public ServiceResponse getCustomDates(@RequestBody XbrlWebFormDto xbrlWebFormDto) {
		ServiceResponse response = null;
		try {
			if (Objects.isNull(xbrlWebFormDto)) {
			...
			listOfOption = getOptionData(xbrlWebFormDto);
	...
	public ServiceResponse getPDRIII60DaysCustomDates(@RequestBody XbrlWebFormDto xbrlWebFormDto) {
		ServiceResponse response = null;
		try {
			List<String> dateList = getPreviousDatesExcludeWeekendsHolidays(GeneralConstants.DAYS_60.getConstantIntVal(), xbrlWebFormDto.getEndDate(), xbrlWebFormDto.getSessionDateFormat());
			List<Option> listOfOption = new ArrayList<>();
			listOfOption = getSortedOptionData(dateList, xbrlWebFormDto.getSessionDateFormat());
			if (CollectionUtils.isEmpty(listOfOption)) {
	...
	public ServiceResponse getPDRIII250DaysCustomDates(@RequestBody XbrlWebFormDto xbrlWebFormDto) {
		ServiceResponse response = null;
		try {
			List<String> dateList = getPreviousDatesExcludeWeekendsHolidays(GeneralConstants.DAYS_250.getConstantIntVal(), xbrlWebFormDto.getEndDate(), xbrlWebFormDto.getSessionDateFormat());
			List<Option> listOfOption = new ArrayList<>();
			listOfOption = getSortedOptionData(dateList, xbrlWebFormDto.getSessionDateFormat());
			if (CollectionUtils.isEmpty(listOfOption)) {
	...
	public List<Option> getOptionData(XbrlWebFormDto xbrlWebFormDto) {
		List<Option> listOfOption = new ArrayList<>();
		try {
			ReturnCustomDate returnCustomDate = returnCustomDateRepo.getReturnCustomDate(xbrlWebFormDto.getReturnCode());
			Date reportingEndDate = DateManip.convertStringToDate(xbrlWebFormDto.getEndDate(), xbrlWebFormDto.getSessionDateFormat());
			Date reportingStartDate = DateManip.convertStringToDate(xbrlWebFormDto.getStartDate(), xbrlWebFormDto.getSessionDateFormat());
			...
			long numOfDays;
			if (returnCodeList.contains(xbrlWebFormDto.getReturnCode())) {
				...
				jsonData = xbrlWebFormController.fetchCustomJsonData(reportingStartDate, reportingEndDate, "APR_TO_MAR", xbrlWebFormDto.getSessionDateFormat(), returnCustomDate, null);
				mapper = new ObjectMapper();
				map = mapper.readValue(jsonData, Map.class);
				Date fortNightreportingEndDate = null;
				Date fortNightreportingStartDate = null;
				if (map.size() > 3) {
					fortNightreportingEndDate = DateManip.convertStringToDate(map.get(GeneralConstants.THIRD_FORTNIGHT_END_DATE.getConstantVal()), xbrlWebFormDto.getSessionDateFormat());
					fortNightreportingStartDate = DateManip.convertStringToDate(map.get(GeneralConstants.LAST_MONTH_LAST_FORTNIGHT_END_DATE.getConstantVal()), xbrlWebFormDto.getSessionDateFormat());
				} else {
					fortNightreportingEndDate = DateManip.convertStringToDate(map.get(GeneralConstants.SECOND_FORTNIGHT_END_DATE.getConstantVal()), xbrlWebFormDto.getSessionDateFormat());
					fortNightreportingStartDate = DateManip.convertStringToDate(map.get(GeneralConstants.LAST_MONTH_LAST_FORTNIGHT_END_DATE.getConstantVal()), xbrlWebFormDto.getSessionDateFormat());
				}
				...
				mapper = new ObjectMapper();
				map = mapper.readValue(jsonData, Map.class);
				LocalDate lastMonthFriday = getPreviousMonthLastFriday(reportingStartDate, xbrlWebFormDto.getSessionDateFormat());
				Date currentMonthLastFriday = null;
				if (map.size() == 3) {
					currentMonthLastFriday = DateManip.convertStringToDate(map.get(GeneralConstants.THIRD_ALTERNATIVE_FRIDAY.getConstantVal()), xbrlWebFormDto.getSessionDateFormat());
				} else {
					currentMonthLastFriday = DateManip.convertStringToDate(map.get(GeneralConstants.SECOND_ALTERNATIVE_FRIDAY.getConstantVal()), xbrlWebFormDto.getSessionDateFormat());
				}
			...
			if (!CollectionUtils.isEmpty(listOfDates)) {
				...
				for (LocalDate listOfDatesObj : listOfDates) {
					option = new Option();
					String formattedDate = listOfDatesObj.format(DateTimeFormatter.ofPattern(xbrlWebFormDto.getSessionDateFormat()));
