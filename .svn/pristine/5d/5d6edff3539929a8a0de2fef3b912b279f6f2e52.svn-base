...
public class SdmxElementDependencyController {
	...
	@PostMapping(value = "/fetchDependencyByActiveStatus/{userId}")
	public ServiceResponse fetchElementDependency(@RequestHeader("JobProcessingId") String jobProcessId, @RequestHeader("IsActive") Boolean isActive, @PathVariable("userId") Long userId, @RequestBody SdmxElementDependencyTypeBean sdmxElementDependencyTypeBeans) {
		LOGGER.info("START - Fetch Element Dependency by Active status request received with Job Processing ID : " + jobProcessId);
		...
		LOGGER.info("End - Fetch Element Dependency by Active status request completed with Job Processing ID : " + jobProcessId);
