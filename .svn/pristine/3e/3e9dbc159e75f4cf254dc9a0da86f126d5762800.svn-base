...
public class FilingCalendarMailSchedulerController {
	...
	//	@GetMapping(value = "/sendMailNotification")
			...
			for (Return returnItr : returnList2) {
				...
				if (retObj != null) {
					if (retObj.getReturnPropertyIdFk() != null && !CollectionUtils.isEmpty(returnProList)) {
						returnProList = returnProList.stream().filter(x -> x.getReturnProprtyIdFK() != null).filter(x -> x.getReturnProprtyIdFK().getReturnProprtyId().equals(retObj.getReturnPropertyIdFk().getReturnProprtyId())).collect(Collectors.toList());
				...
				if (returnPropertyValList.isEmpty()) {
					...
					if (filingCalendarData != null) {
						if ("1".equalsIgnoreCase(filingCalendarData.getSendMail())) {
							...
							notificationDate = getNotificationDate(filingCalendarData, endDate);
							//	notificationDate = filingCalendarController.getWindowEndDate(endDate,(int) (long)filingCalendarData.getEmailNotificationDays(),filingCalendarData.getFilingWindowExtensionStart()+1+filingCalendarData.getGraceDays());
	...
	private String getNotificationDate(FilingCalendar filingCalendarData, String endDate) throws ParseException {
		String notificationDate = null;
		List<String> setFilingDatails = new ArrayList<>();
		SimpleDateFormat format1 = new SimpleDateFormat("E MMM dd HH:mm:ss zzzz yyyy", Locale.ENGLISH);
		SimpleDateFormat format2 = new SimpleDateFormat("dd-MM-yyyy");
		try {
			setFilingDatails.add(filingCalendarData.getReturnFrequencyId().toString());
			setFilingDatails.add(filingCalendarData.getReturnId().toString());
			if (filingCalendarData.getReturnPropertyVal() != null)
				setFilingDatails.add(filingCalendarData.getReturnPropertyVal().getReturnProprtyValId().toString());
			else
				setFilingDatails.add(null);
			setFilingDatails.add(endDate);
			setFilingDatails.add(format1.format(format2.parse(endDate)));
			setFilingDatails.add(DateConstants.DD_MM_YYYY.getDateConstants());
			ServiceResponse serviceResponse = filingCalendarController.viewFilingCalendarInfo(jobProcessingId, setFilingDatails);
			if (serviceResponse.isStatus()) {
				@SuppressWarnings("unchecked")
				List<String> filingDetailsList = (List<String>) serviceResponse.getResponse();
				notificationDate = filingDetailsList.get(7);
			}
		} catch (Exception e) {
			logger.error("Exception in getFilingInformation for JobProcessingId " + jobProcessingId + "Exception is" + e);
		}
