...
public class AuditFirmProcessor implements Serializable {
	...
	public ServiceResponse addAuditFirm(AuditFirm auditFirm) {
		...
		try {
			...
			if (!auditFirm.isEditFlag()) {
				...
				try {
					AuditFirm auditFirmModObj = new AuditFirm();
					auditFirmModObj.setAuditFirmDefaultName(auditFirmObj.getAuditFirmDefaultName());
					auditFirmModObj.setAuditFirmId(auditFirmObj.getAuditFirmId());
					auditFirmModObj.setAuditFirmRegNo(auditFirmObj.getAuditFirmRegNo());
					auditFirmModObj.setAuditFirmWebsite(auditFirmObj.getAuditFirmWebsite());
					auditFirmModObj.setBusinessAddress(auditFirmObj.getBusinessAddress());
					auditFirmModObj.setCountryName(auditFirmObj.getCountry().getCountryName());
					auditFirmModObj.setMobileNo(auditFirmObj.getMobileNo());
					auditFirmModObj.setEmailAddress(auditFirmObj.getEmailAddress());
					auditFirmModObj.setPhoneNo(auditFirmObj.getPhoneNo());
					auditFirmModObj.setIsActive(auditFirmObj.getIsActive());

					AuditFirmMod auditFirmMod = new AuditFirmMod();
					auditFirmMod.setAuditFirmModId(auditFirmObj);
					String jsonResult = new Gson().toJson(auditFirmModObj);
	...
	public AuditFirmInfo checkAuditFirmExists(String firmRegNo) {
		LOGGER.info("Checking whether the Firm Registeration Number exists in the system: AuditFirmProcessor.checkAuditFirmExists");
		try {
	 ...
	 * 
	 */
	public List<AuditFirm> getActiveAuditorFirmList(String langCode) {
		LOGGER.info("Fetching the Audit Firm List: AuditFirmProcessor.getAuditorFirmList");
		List<AuditFirm> auditFirmList = new ArrayList<>();
		List<AuditFirmInfo> auditFirmInfoList = new ArrayList<>();
		...
		try {
			AuditFirm auditFirm;
			auditFirmInfoList = auditFirmRepo.getActiveAuditorFirmList();
	...
	public boolean addAuditFirmId(Long firmId) throws SQLException {
		try (Connection con = datasource.getConnection(); CallableStatement stmt = con.prepareCall(GeneralConstants.SP_INSERT_AUDITOR_LBL.getConstantVal());) {
	...
	public ServiceResponse updateAuditFirmLabel(AuditFirm auditFirm) {
		try {
			...
			auditFirmLabelObj.setAuditFirmRegNo(auditFirmLabel.getAuditFirmIdFk().getAuditFirmRegNo());
	...
	public AuditFirm getPreviousAuditFirmById(Long auditFirmId) {
		AuditFirm auditFirm = null;
		AuditFirmMod auditFirmMod = new AuditFirmMod();
		try {
			auditFirmMod = auditFirmModRepo.getPreviousAuditFirmById(auditFirmId);
			auditFirm = JsonUtility.getGsonObject().fromJson(auditFirmMod.getAuditFirmModJson(), AuditFirm.class);
		} catch (Exception e) {
			LOGGER.error(ErrorCode.EC0033.toString());
			throw new ServiceException(ErrorConstants.ERROR_MSG_SERVICE.getConstantVal(), e);
		}
	...
	public AuditFirm getPreviousAuditFirmLabelById(Long auditFirmId) {
		AuditFirmLabel auditFirmLabel = null;
		AuditFirmLabelMod auditFirmLabelMod = new AuditFirmLabelMod();
		AuditFirm auditFirm = new AuditFirm();

		try {
			auditFirmLabelMod = auditFirmLabelModRepo.getPreviousAuditFirmLabelById(auditFirmId);
			auditFirmLabel = JsonUtility.getGsonObject().fromJson(auditFirmLabelMod.getAuditFirmLabelModJson(), AuditFirmLabel.class);
			auditFirm.setLanguageName(auditFirmLabelMod.getAuditFirmLabelModId().getLanguageMaster().getLanguageName());
			auditFirm.setAuditFirmRegNo(auditFirmLabel.getAuditFirmRegNo());
			auditFirm.setAuditFirmName(auditFirmLabel.getAuditFirmNameLabel());
