...
public interface SdmxReturnModelInfoRepo extends JpaRepository<SdmxReturnModelInfoEntity, Long> {
	...
	SdmxReturnModelInfoEntity findActiveEntityByModelCodesFkNCellRef(SdmxModelCodesEntity modelCodesIdFk, Integer returnCellRef);
	...
	SdmxReturnModelInfoEntity findByReturnSheetTemplateIdNCellRef(SdmxReturnSheetInfoEntity returnSheetInfoIdFk, Integer returnCellRef);
	...
	SdmxReturnModelInfoEntity findEntityByReturnTemplateNCellRef(ReturnTemplate returnTemplateId, Integer returnCellRef, Long returnPreviewIdFk);
	...
	@Query("SELECT new com.iris.sdmx.exceltohtml.bean.SdmxReturnModelMappingBean(srsi.returnTemplateIdFk.returnTemplateId, srmi.returnCellRef, srsi.returnPreviewIdFk, srp.ebrVersion) FROM SdmxReturnSheetInfoEntity srsi, SdmxReturnModelInfoEntity srmi, SdmxReturnPreviewEntity srp " + "WHERE srmi.returnSheetInfoIdFk = srsi.returnSheetInfoId and srsi.returnTemplateIdFk.returnTemplateId IN(:returnTemplateIds) and srmi.isActive=1" + " and srsi.returnPreviewIdFk = srp.returnPreviewTypeId and srp.isActive=1 and srp.isPublished=1")
	...
	Boolean fetchIsMandatoryByReturnTemplateNCellRefId(Integer returnCellRef, ReturnTemplate returnTemplateId, Long returnPreviewIdFk);

	@Query("SELECT new com.iris.sdmx.exceltohtml.bean.SdmxReturnModelMappingBean(ret.returnId, modelInfo.returnCellRef, " + " modeCodes.modelCode, modelInfo.isMandatory, ele.dsdCode, ele.elementVer) from SdmxReturnSheetInfoEntity sheet, SdmxReturnModelInfoEntity modelInfo, " + " SdmxModelCodesEntity modeCodes, " + " ReturnTemplate temp, Return ret, SdmxElementEntity ele where sheet.returnSheetInfoId = modelInfo.returnSheetInfoIdFk.returnSheetInfoId " + " and ele.elementId = modeCodes.elementIdFk.elementId and sheet.returnTemplateIdFk.returnTemplateId = temp.returnTemplateId " + " and ret.returnId = temp.returnObj.returnId and modeCodes.modelCodesId = modelInfo.modelCodesIdFk.modelCodesId " + " and modelInfo.isActive = 1 and modeCodes.isActive = 1 and sheet.returnTemplateIdFk.returnTemplateId IN(:returnTemplateIds)")
	List<SdmxReturnModelMappingBean> fetchReturnModelMapping(List<Long> returnTemplateIds);

	@Query("SELECT cellFormula FROM SdmxReturnModelInfoEntity WHERE returnSheetInfoIdFk " + "IN(SELECT returnSheetInfoId FROM SdmxReturnSheetInfoEntity WHERE returnPreviewIdFk=:returnTemplateId) " + "and cellFormula != null")
	List<String> fetchFormulaBuilderJsonFromTemplateId(Long returnTemplateId);

	@Query("SELECT modelCode FROM SdmxModelCodesEntity WHERE modelCodesId = (SELECT modelCodesIdFk " + "FROM SdmxReturnModelInfoEntity WHERE returnSheetInfoIdFk " + "IN(SELECT returnSheetInfoId FROM SdmxReturnSheetInfoEntity WHERE returnPreviewIdFk=:returnTemplateId) " + "and returnCellRef =:cellNo and isActive=true)")
	String fetchDmIdFromCellNumberNPreviewId(Integer cellNo, Long returnTemplateId);

	//@Query("FROM SdmxReturnModelInfoEntity srmi where srmi.returnSheetInfoIdFk IN(SELECT rsi.returnSheetInfoId FROM SdmxReturnSheetInfoEntity rsi where rsi.returnPreviewIdFk=:returnPreviewIdFk) and srmi.returnCellRef=:returnCellRef and srmi.isActive=true")
	//SdmxReturnModelInfoEntity findEntityByPreviewIdNCellRef(Integer returnCellRef, Long returnPreviewIdFk);

	@Query("FROM SdmxReturnModelInfoEntity srmi where srmi.returnSheetInfoIdFk IN(SELECT rsi.returnSheetInfoId FROM SdmxReturnSheetInfoEntity rsi where rsi.returnPreviewIdFk=:returnPreviewIdFk) and srmi.isActive=true")
	List<SdmxReturnModelInfoEntity> findEntityByPreviewId(Long returnPreviewIdFk);

	@Query("SELECT srmie.returnCellRef " + " FROM SdmxReturnModelInfoEntity srmie, " + " SdmxReturnSheetInfoEntity rsi, " + " SdmxReturnPreviewEntity srp, " + " SdmxModelCodesEntity smce WHERE " + " srp.returnPreviewTypeId=:returnPreviewIdFk" + " and srp.isActive=true " + " and rsi.returnPreviewIdFk=srp.returnPreviewTypeId " + " and rsi.returnSheetInfoId=srmie.returnSheetInfoIdFk.returnSheetInfoId " + " and srmie.modelCodesIdFk.modelCodesId = smce.modelCodesId " + " and smce.modelDim like \'%\\\"dependencyType\\\": \\\"DEPENDENT\\\"%\' " + " and smce.isActive = true")
	List<Integer> fetchTheDepedentCellNumbers(Long returnPreviewIdFk);
