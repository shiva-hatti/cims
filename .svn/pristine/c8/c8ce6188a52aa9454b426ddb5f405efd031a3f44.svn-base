...
public interface GroupMasterRepo extends JpaRepository<GroupMaster, Integer> {
	...
	@Query("update GroupMaster SET remark = :remark, alternateName = :alternateName, mobileNumber = :mobileNumber, groupIdFk = :groupIdFk " + " where groupCode = :groupCode")
	...
	@Query("From GroupMaster where UPPER(groupCode) LIKE %:groupCode% order by groupCode asc")
	List<GroupMaster> getGroupDetails(@Param("groupCode") String groupCode);
