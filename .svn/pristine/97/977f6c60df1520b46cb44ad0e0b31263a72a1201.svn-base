...
public class ElementDimensionApprovalController {
	...
	public ServiceResponse isDimensionListPending(@RequestHeader(name = "JobProcessingId") String jobProcessId, @RequestBody ElementDimensionBean elementDimensionBean, @PathVariable(name = "elementVersion") String elementVersion, @PathVariable(name = "agencyCode") String agencyCode) {
		LOGGER.info("Request received to check code list master pending or not for job processing Id : " + jobProcessId);
		...
		try {
			...
			return new ServiceResponseBuilder().setStatus(true).setResponse(elementDimApprovalService.isDimensionListPending(elementDimensionBean.getDimensionmasterBeans(), agencyCode, elementVersion)).build();
	...
	private void validateInputRequestForRecordPending(String jobProcessingId, ElementDimPendingBean elementDimPendingBean) throws ApplicationException {
		if (UtilMaster.isEmpty(jobProcessingId) || UtilMaster.isEmpty(elementDimPendingBean.getElementVersion()) || UtilMaster.isEmpty(elementDimPendingBean.getDsdCode()) || UtilMaster.isEmpty(elementDimPendingBean.getUserId()) || UtilMaster.isEmpty(elementDimPendingBean.getRoleId())) {
